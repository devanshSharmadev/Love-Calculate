...
def love(request):
    return render(request,"love.html",{'msg':'','lc':''})

def submit(request):
    boy=request.GET['bname']
    girl=request.GET['gname']
    ...
    if lovecount in impossible:
        ...
        msg=("""
          ...
          NEVER...Theek hai {0} ab dusri ladki dhundo ;)!!!
          """.format(boy, girl))
        lovecount = -100
    ...
    # DATABASE MANAGEMENT
    conn = mysql.connect(host='localhost', port=3306, user='root', password='pandit', db='lovecalculator')
    cmd = conn.cursor()
    q="insert into category (boy,girl) values ('{0}','{1}')".format(boy,girl)
    cmd.execute(q)
    conn.commit()
    conn.close()
    return render(request,"love.html",{'msg':msg,'lc':lovecount})
